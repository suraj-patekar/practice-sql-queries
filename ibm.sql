...
-- find the number of countries offering discout to the family with size greater then min_size requirement
select count(*) as max_discount_countries from countries where min_size <= (select max(FAMILY_SIZE) from families);


-- find how many countries are offering discount based on the family size 
with cte as (select f.name, f.family_size,c.name as country_name from families f inner join countries c
on f.family_size between c.min_size and c.max_size)
select name, count(*) from cte
group by name
order by count(*) desc;
